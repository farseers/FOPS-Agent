...
│                                                                  │
│  ┌──────────────────┐     ┌──────────────────────────────────┐  │
│  │ ContainerManager │────>│ CollectorManager (Observer)    │  │
│  │                  │     │                                  │  │
...
### 2. CollectorManager (`watcher/manager.go`)

文件监视器管理器，实现 `container.Observer` 接口：
- 预创建全局 HTTPUploader（每个 collector 一个）
- 管理每个容器的 ContainerCollector
- 容器创建时创建对应的监视器
- 容器销毁时清理对应的监视器

### 3. ContainerCollector (`watcher/container_watcher.go`)
...
1. 容器启动事件
   ...
   → CollectorManager.OnContainerAdd() → NewContainerCollector()
