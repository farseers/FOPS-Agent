...
func CollectLog(wsServer string) {
	...
	logCollector.OnLogFile(func(logFile *collector.LogFile) error {
		lstLogData := collections.NewList[flog.LogData]()
		...
		bodyByte, _ := snc.Marshal(UploadRequest{List: lstLogData})
